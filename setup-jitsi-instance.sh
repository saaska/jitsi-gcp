...
mkdir /usr/share/jitsi
mv docker-jitsi-meet/* /usr/share/jitsi/docker-jitsi-meet
...
sudo mkdir /usr/share/jitsi/certs
...
curl -s https://secretmanager.googleapis.com/v1/projects/$PROJECT_ID/secrets/$KEY_SECRET_NAME/versions/latest:access  \
   ...
   | jq -r ".payload.data" | base64 --decode | sudo tee /usr/share/jitsi/certs/key.pem > /dev/null
curl -s https://secretmanager.googleapis.com/v1/projects/$PROJECT_ID/secrets/$FULLCHAIN_SECRET_NAME/versions/latest:access  \
   --request "GET" --header "authorization: Bearer $TOKEN" --header "content-type: application/json" --silent \
   | jq -r ".payload.data" | base64 --decode | sudo tee /usr/share/jitsi/certs/fullchain.pem > /dev/null
echo SETUP: Retrieved SSL certificates>> /var/log/setup-jitsi.log

# Set up Jitsi config
cd /usr/share/jitsi/docker-jitsi-meet
cp env.example .env
./gen-passwords.sh
...
sed -i -e "1,/prosody/ s%volumes:%volumes:\n            - /usr/share/jitsi/certs/fullchain.pem:/config/keys/cert.crt\n            - /usr/share/jitsi/certs/key.pem:/config/keys/cert.key%" docker-compose.yml
